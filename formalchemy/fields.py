...
class FieldRenderer(object):
    ...
    def render_readonly(self, **kwargs):
        ...
        if isinstance(value, unicode):
            return '<span id="%s">%s</span>' % (self.name, value)
        value = self.stringify_value(value, as_html=True)
        return '<span id="%s">%s</span>' % (self.name, value)
...
class DateFieldRenderer(FieldRenderer):
    ...
    def render_readonly(self, **kwargs):
        value = self.raw_value
        return value and '<span id="%s">%s</span>' % (self.name, value.strftime(self.format)) or ''
...
class TimeFieldRenderer(FieldRenderer):
    ...
    def render_readonly(self, **kwargs):
        value = self.raw_value
        return isinstance(value, datetime.time) and '<span id="%s">%s</span>' % (self.name, value.strftime(self.format)) or ''
    def _render(self, **kwargs):
        data = self.params
        value = self.field.model_value
